...
$(document).ready(function() {
    // Load products
    $.get('<?php echo e(route("quotations.products")); ?>', function(response) {
        if (response.success) {
    ...
    invoiceTable = $('#invoice-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '<?php echo e(route("invoice-report")); ?>',
            type: 'GET',
            data: function(d) {
...
function viewInvoice(id) {
    $.get('<?php echo e(url("invoices")); ?>/' + id, function(response) {
        if (response.success) {
...
function recordPayment() {
    ...
    $.ajax({
        url: '<?php echo e(url("invoices")); ?>/' + invoiceId + '/payment',
        type: 'POST',
        data: {
            _token: '<?php echo e(csrf_token()); ?>',
            amount: totalPayment,
...
function printInvoice(id) { window.open('<?php echo e(url("invoices")); ?>/' + id + '?print=1', '_blank'); }
...
function deleteInvoice(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '<?php echo e(url("invoices")); ?>/' + id, type: 'DELETE', headers: {'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'},
                success: function(response) {
...
<?php echo $__env->make('layout.mainlayout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\laundry\resources\views/invoice-report.blade.php ENDPATH**/ ?>