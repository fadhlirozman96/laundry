...
<body>
    ...
    <div class="header">
        <div class="logo-section">
            <h1>{{ $quotation->store->name ?? 'Store Name' }}</h1>
            <p>{{ $quotation->store->address ?? '' }}</p>
            <p>{{ $quotation->store->phone ?? '' }} {{ $quotation->store->email ? '| ' . $quotation->store->email : '' }}</p>
        </div>
        <div class="quotation-info">
            <h2>QUOTATION</h2>
            <p class="ref">{{ $quotation->quotation_number }}</p>
            <p><strong>Date:</strong> {{ $quotation->created_at->format('d M Y') }}</p>
            <p><strong>Valid Until:</strong> {{ $quotation->valid_until ? \Carbon\Carbon::parse($quotation->valid_until)->format('d M Y') : '-' }}</p>
            <p>
    ...
    <div class="addresses">
        <div class="address-box">
            <h3>Bill To</h3>
            <p class="name">{{ $quotation->customer_name ?: 'Walk in Customer' }}</p>
        ...
        <div class="address-box">
            <h3>From</h3>
            <p class="name">{{ $quotation->store->name ?? 'Store' }}</p>
            @if($quotation->store && $quotation->store->address)
                <p>{{ $quotation->store->address }}</p>
            @endif
            @if($quotation->store && $quotation->store->phone)
                <p>Phone: {{ $quotation->store->phone }}</p>
            @endif
            @if($quotation->store && $quotation->store->email)
                <p>Email: {{ $quotation->store->email }}</p>
            @endif
        </div>
    </div>
    ...
    <div class="totals">
        <table>
            <tr>
                <td>Subtotal:</td>
                <td>MYR {{ number_format($quotation->subtotal, 2) }}</td>
            </tr>
            <tr>
                <td>Tax:</td>
                <td>MYR {{ number_format($quotation->tax ?? 0, 2) }}</td>
            </tr>
            <tr>
                <td>Discount:</td>
                <td>- MYR {{ number_format($quotation->discount ?? 0, 2) }}</td>
            </tr>
            <tr class="grand-total">
                <td>Total:</td>
                <td>MYR {{ number_format($quotation->total, 2) }}</td>
            </tr>
        </table>
    </div>

    @if($quotation->notes && $quotation->notes != '<p><br></p>')
