                    ...
                    <table class="table datanew">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $type->category ?? '-' }}</td>
                                <td>MYR {{ number_format($type->default_price, 2) }}</td>
                                <td>
                                    @if($type->is_active)
                                ...
                                <td>
                                    <div class="edit-delete-action">
                                        <a class="me-2 p-2" href="javascript:void(0);" 
                                           onclick="editGarment({{ $type->id }}, '{{ addslashes($type->name) }}', '{{ addslashes($type->category) }}', '{{ addslashes($type->description) }}', {{ $type->default_price }}, {{ $type->is_active ? 1 : 0 }})">
                                            <i data-feather="edit" class="feather-edit"></i>
                                        </a>
                                        <a class="confirm-text p-2" href="javascript:void(0);" onclick="deleteGarment({{ $type->id }})">
...
<script>
    ...
    $('#add-garment-form').on('submit', function(e) {
        e.preventDefault();
        $.ajax({
            url: '{{ route("laundry.garment-types.store") }}',
            type: 'POST',
            data: $(this).serialize(),
            success: function(response) {
